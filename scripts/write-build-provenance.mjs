 ...
 *
 * The Sentry DSN is not in source. It arrives here through OPENWIZARDAI_SENTRY_DSN,
 ...
 * were found reporting into the smash-labs/openwizardai project simultaneously, one
 ...
 *
 * Forking OpenWizardAI? Set OPENWIZARDAI_SENTRY_DSN to your OWN DSN and your crash reports
...
const dsn = (process.env.OPENWIZARDAI_SENTRY_DSN || '').trim();

if (verifyOnly) {
	if (!dsn) {
		console.error(
			'build-provenance: OPENWIZARDAI_SENTRY_DSN is not set. An official build would ship with\n' +
				'crash reporting disabled. Set the OPENWIZARDAI_SENTRY_DSN repository secret, or drop the\n' +
				'--verify step if this build is intentionally unreported.'
		);
		process.exit(1);
	}
	console.log('build-provenance: OPENWIZARDAI_SENTRY_DSN present.');
	process.exit(0);
}
...
if (!dsn) {
	...
	console.log(
		'build-provenance: no OPENWIZARDAI_SENTRY_DSN, crash reporting disabled for this build.'
	);
	process.exit(0);
}

// `official` marks builds produced by our own pipeline. A fork supplying its own
// DSN gets a working file without claiming to be an official OpenWizardAI build.
const official =
	process.env.OPENWIZARDAI_OFFICIAL_BUILD === '1' || Boolean(process.env.GITHUB_ACTIONS);
