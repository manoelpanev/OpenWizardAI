#!/bin/sh
# openwizardai-p installer (Linux + macOS)
#
# Installs the `openwizardai-p` wrapper, which lets callers use `claude -p` semantics
...
#
# Override the download host with OPENWIZARDAI_BASE_URL, the runtime dir with
# OPENWIZARDAI_P_HOME, or the shim dir with OPENWIZARDAI_P_BIN, before piping into sh.

set -eu

BASE_URL="${OPENWIZARDAI_BASE_URL:-https://github.com/manoelpanev/OpenWizardAI"
INSTALL_DIR="${OPENWIZARDAI_P_HOME:-/usr/local/lib/openwizardai-p}"
SHIM_DIR="${OPENWIZARDAI_P_BIN:-/usr/local/bin}"
MIN_NODE_MAJOR=20
...
if need_root_for "$INSTALL_DIR" || need_root_for "$SHIM_DIR"; then
	if [ "$(id -u)" -eq 0 ]; then
		...
		die "Need write access to ${INSTALL_DIR} and ${SHIM_DIR} but sudo is unavailable. Re-run as root, or set OPENWIZARDAI_P_HOME/OPENWIZARDAI_P_BIN to writable paths."
	fi
fi
...
if [ "$NODE_MAJOR" -lt "$MIN_NODE_MAJOR" ]; then
	die "Node.js $(node -v) is too old. openwizardai-p needs Node >= ${MIN_NODE_MAJOR}."
fi
ok "Node.js $(node -v)"
...
if command -v claude >/dev/null 2>&1; then
	...
	warn "openwizardai-p drives Claude Code, so install + log in to it before use:"
...
STAGE="$(mktemp -d 2>/dev/null || mktemp -d -t openwizardai-p)"
cleanup() { rm -rf "$STAGE" 2>/dev/null || true; }
trap cleanup EXIT INT TERM

info "Staging openwizardai-p"
mkdir -p "$STAGE/bin"

info "Downloading openwizardai-p.js"
download "$BASE_URL/openwizardai-p.js" "$STAGE/bin/openwizardai-p.js"
chmod 0755 "$STAGE/bin/openwizardai-p.js"
ok "openwizardai-p.js"

info "Downloading package.json"
download "$BASE_URL/openwizardai-p.package.json" "$STAGE/package.json"
ok "package.json"
...
# ---- shim ----------------------------------------------------------------
SHIM_PATH="$SHIM_DIR/openwizardai-p"
SHIM_TMP="$STAGE/openwizardai-p.shim"
cat > "$SHIM_TMP" <<EOF
#!/bin/sh
exec node "$INSTALL_DIR/bin/openwizardai-p.js" "\$@"
EOF
chmod 0755 "$SHIM_TMP"
...
# ---- verify --------------------------------------------------------------
INSTALLED_VERSION="$(node "$INSTALL_DIR/bin/openwizardai-p.js" --version 2>/dev/null || echo '?')"
ok "openwizardai-p ${INSTALLED_VERSION}"

printf '\n%sInstalled for all users.%s\n' "$C_BOLD" "$C_RESET"
case ":$PATH:" in
	*":$SHIM_DIR:"*) printf '  Run: %sopenwizardai-p --help%s\n' "$C_BOLD" "$C_RESET" ;;
	*)
		printf '  %s%s is not on your PATH.%s Add it:\n' "$C_YELLOW" "$SHIM_DIR" "$C_RESET"
		printf '    echo '"'"'export PATH="%s:$PATH"'"'"' >> ~/.profile && . ~/.profile\n' "$SHIM_DIR"
		printf '  Then run: %sopenwizardai-p --help%s\n' "$C_BOLD" "$C_RESET"
		;;
esac
