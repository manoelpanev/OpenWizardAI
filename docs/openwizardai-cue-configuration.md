...
description: Complete YAML schema reference for .openwizardai/cue.yaml configuration files.
icon: file-code
---

Cue is configured via a `.openwizardai/cue.yaml` file placed inside the `.openwizardai/` directory at your project root. The engine watches this file for changes and hot-reloads automatically.
...
OpenWizardAI discovers this file automatically when the Cue Encore Feature is enabled. Each agent that has a `.openwizardai/cue.yaml` in its project root gets its own independent Cue engine instance.

<Note>
**One cue.yaml per agent project root.** The engine reads ONLY `<projectRoot>/.openwizardai/cue.yaml` for each agent - it does not walk parent directories and does not fall back to any ancestor or workspace-wide config. If your fleet has agents at multiple project roots, you maintain one cue.yaml per root. See [Multi-root pipelines](#multi-root-pipelines-agents-in-different-project-roots) below.
</Note>
...
When two or more agents are registered against the same project directory (for example, one agent using Opus and another using Sonnet, both pointing at the same vault), every _unowned_ subscription (one without an explicit `agent_id`) would otherwise fire once per agent. OpenWizardAI resolves this as follows:
...
**Orchestration "at the root."** If you have an orchestrator agent whose project root sits above the worker agents in the filesystem, the orchestrator's own `.openwizardai/cue.yaml` is naturally where fan-in / synthesis subscriptions land - because it owns those subscriptions, not because it is "the root." Workers' triggers still live in each worker's own cue.yaml.
...
**The visual editor is the easy path.** When you save a multi-root pipeline from the Pipeline Graph, OpenWizardAI automatically partitions the subscriptions by owning agent's project root and writes one yaml per participating cwd. If you find yourself authoring a multi-root pipeline by hand and it gets fiddly, building it in the Pipeline Graph and letting it emit the per-cwd files is the supported path.
...
| `event`  | string | One of the eleven [event types](./openwizardai-cue-events)                    |
...
| `fire_at`                  | string (ISO-8601) | -        | Wall-clock fire moment with timezone offset. Required for `time.once`. Authored via `openwizardai-cli cue schedule`                                                                                                                       |
...
| `filter`                   | object            | -        | Payload conditions (see [Filtering](./openwizardai-cue-advanced#filtering))                                                                                                                                                               |
...
| `command`                  | object            | -        | Command spec when `action: command`. Fields include `mode` (`shell` \| `cli`), `shell`/`cli` invocation, and related options (see [Command Nodes](./openwizardai-cue-advanced))                                                           |
...
```yaml
prompt_file: .openwizardai/prompts/my-prompt.md
```

File paths are resolved relative to the project root. Prompt files support the same `{{VARIABLE}}` template syntax as inline prompts. Using `prompt_file` keeps your `cue.yaml` clean when prompts are long or complex - the Pipeline Graph uses this approach by default, storing prompt files in `.openwizardai/prompts/`.
...
subscriptions:
  - name: My Pipeline
    ...
    schedule_times:
      - '09:00'
    prompt_file: .openwizardai/prompts/my-pipeline-daily.md

  - name: My Pipeline-chain-1
    event: time.scheduled
    schedule_times:
      - '17:00'
    prompt_file: .openwizardai/prompts/my-pipeline-eod.md
```
...
If an AI agent writes `cue.yaml` directly (without using the visual editor), include all of the following so OpenWizardAI reconstructs the graph correctly:
...
| `"fire_at" is required`                    | `time.once` events need an ISO-8601 timestamp with timezone offset (use `openwizardai-cli cue schedule`) |
