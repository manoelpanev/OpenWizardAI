...
The `triggeredBy` field contains the subscription name that triggered the completing run. It supports glob patterns (e.g., `triggeredBy: "deploy-*"`). See [Selective Chaining](./openwizardai-cue-examples#selective-chaining-with-triggeredby) for a full walkthrough.
...
Cue prompts also have access to all standard OpenWizardAI template variables (like `{{PROJECT_ROOT}}`, `{{TIMESTAMP}}`, etc.) - the same variables available in Auto Run playbooks and system prompts.
