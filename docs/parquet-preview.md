...
description: Open a Parquet file as a live, filterable table. OpenWizardAI reads only the columns and row groups your filter actually needs, so a multi-gigabyte file opens instantly.
icon: table
---

Double-click a `.parquet` file and OpenWizardAI opens it as a table you can filter, sort, and read, not as a "binary file" card.

Parquet files are routinely larger than memory, so OpenWizardAI never loads one. It reads the file's footer to learn the schema, then fetches only the rows on screen and only the columns you are looking at. Opening a 20 GB file costs the same few kilobytes as opening a 2 MB one.
...
For how this compares to the other formats OpenWizardAI opens specially - jq for JSON, row filtering for CSV - see [File Formats](/file-formats).
...
Misspell a column and OpenWizardAI tells you which one you meant, with a button to fix it. Click the `?` beside the box for the same list of examples, and click any of them to try it.
...
- Click the eye to hide a column. Hidden columns are dropped from the query, so OpenWizardAI stops reading them entirely - on a wide table this makes everything faster, not just tidier.
...
While a filter is still scanning, the count reads `1,204+ of 8,412,004 rows match`. The `+` means "at least" - OpenWizardAI shows you rows as soon as it finds them rather than making you wait for a full pass. The exact total fills in behind you.

On a very large file the exact count is not started automatically; click **Count all matches** in the footer when you want it.

If a filter matches an enormous number of rows, OpenWizardAI stops collecting at its internal cap and says so. Narrow the filter to see the rest.
...
A Parquet file on an [SSH remote](/ssh-remote-execution) opens too, but it is the one case where the format's usual trick does not apply. Everything above works because OpenWizardAI reads scattered byte ranges out of the file, and an SSH shell has no way to serve those. So a remote file has to be copied across in full before any of it can be read.

OpenWizardAI copies it in 4 MB blocks into a temporary cache and shows a progress bar while it does. Re-opening the same unchanged file is free; a file that changed on the remote is fetched again.

**The limit is 32 MB.** Above that OpenWizardAI refuses and tells you to copy the file locally, which is the honest answer: 32 MB is already ~43 MB over the wire once base64-encoded, and a larger file means minutes of waiting to open something you may only want a few rows of. Copy it to your machine and it opens instantly, with no size limit at all.

## What is not supported

- **Editing.** The preview is read-only.
- **Encrypted Parquet files.** OpenWizardAI will report that it could not open the file rather than showing partial data.
