...
description: Detailed reference for all eleven OpenWizardAI Cue event types with configuration, payloads, and examples.
icon: calendar-check
---

Cue supports eleven event types. Each type watches for a different kind of activity and produces a payload that can be injected into prompts via [template variables](./openwizardai-cue-advanced#template-variables).

## app.startup

Fires exactly once when the OpenWizardAI application launches. Ideal for workspace setup, dependency installation, health checks, or any initialization that should happen once per session.
...
<Note>
`time.scheduled` and `time.heartbeat` subscriptions are also **Scheduled Tasks**: they appear in the Cue modal's Scheduled Tasks tab and can be created, re-timed, paused, and cancelled with `openwizardai-cli cue schedule --daily-at` / `--every`, without hand-editing YAML.
</Note>
...
| Field                      | Type    | Default | Description                                                                                                                                                                                                                                                           |
| -------------------------- | ------- | ------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `grace_minutes`            | number  | `360`   | Missed-fire grace window in minutes. If OpenWizardAI is closed when `fire_at` arrives, the sub fires on next startup as long as the current time is within this window of `fire_at`. Past the window, the sub is logged as expired and self-destructs without firing. |
...
- The CLI command `openwizardai-cli cue schedule` is the supported way to create `time.once` subs - it handles ISO formatting, timezone offsets, `target_node_key` generation, and writes directly to the owning agent's `.openwizardai/cue.yaml`. **Do not hand-author `time.once` subscriptions.**
...
openwizardai-cli cue schedule --in 20m --agent <agent-id> --prompt "Check the deploy status."

# Schedule via absolute timestamp + notify-only
openwizardai-cli cue schedule --at "2026-05-22 16:00" --agent <agent-id> --notify --sticky --message "Push rc branch"

# List every scheduled task across agents (one-shot and repeating)
openwizardai-cli cue schedule --list

# Only the one-shots
openwizardai-cli cue schedule --list --kind once

# Move the fire time
openwizardai-cli cue schedule --reschedule tasks-once-push-rc-reminder --at "2026-05-22 18:00"

# Cancel a pending task by sub name
openwizardai-cli cue schedule --cancel tasks-once-push-rc-reminder
```

The same tasks are listed and editable in the app under **OpenWizardAI Cue → Scheduled Tasks** (`openwizardai-cli open cue --tab scheduled`). The tab and the CLI share one module, so neither can drift from the YAML.
...
The `changeType` field is also available in [filters](./openwizardai-cue-advanced#filtering).

---

## agent.completed

Fires when another OpenWizardAI agent finishes a task. This is the foundation for agent chaining - building multi-step pipelines where one agent's completion triggers the next.
...
- **Multiple sources** (list): Waits for **all** named agents to complete before firing (fan-in). See [Fan-In](./openwizardai-cue-advanced#fan-in)
...
These fields are also available in [filters](./openwizardai-cue-advanced#filtering).

The `triggeredBy` field is particularly useful when a source agent has multiple Cue subscriptions but you only want to chain from a specific one. See [Selective Chaining](./openwizardai-cue-examples#selective-chaining-with-triggeredby) for a complete example.
...
| `{{CUE_GH_REPO}}`        | Repository in `owner/repo` format | `manoelpanev/OpenWizardAI`            |
...
Fires only when explicitly triggered from the command line via `openwizardai-cli cue trigger <name>`. Unlike other event types, `cli.trigger` has no background watcher or poller - it waits for a manual invocation.

**No additional fields required** - just `name`, `event`, `prompt`, and optionally `enabled`.

**Behavior:**

- Does nothing on its own - only fires when you run `openwizardai-cli cue trigger <subscription-name>`
...
openwizardai-cli cue trigger deploy --json
```
