...
Encore Features are OpenWizardAI's system for shipping powerful capabilities behind a single toggle. A capability starts life as a plugin: gated, off, and opt-in. Once it has earned its place in the core experience it graduates to an Encore Feature and ships **on by default**.
...
| [OpenWizardAI Symphony](./symphony)    | `Opt+Cmd+Y` / `Alt+Ctrl+Y`     | Contribute to open source by donating AI tokens                                                  |
| [OpenWizardAI Cue](./openwizardai-cue) | `Opt+Q` / `Alt+Q`              | Event-driven automation: file changes, timers, agent chaining, GitHub polling, and task tracking |
...
See the [Encore Features contributor guide](https://github.com/manoelpanev/OpenWizardAI/blob/main/CONTRIBUTING.md#encore-features-feature-gating) for the full implementation checklist, architecture details, and the canonical reference implementation (Director's Notes).
