...
Three chains in one pipeline. Replace `<remy-agent-id>`, `<evaluator-agent-id>`, `<analyst-agent-id>`, and `<applier-agent-id>` with real agent UUIDs (`openwizardai-cli list agents`). The four agents can be the same model - they just need separate sessions so their contexts stay clean.
...
Alternative: use a Command node (`action: command, mode: shell`) to extract the run transcript from OpenWizardAI's session history file. The session history JSON lives at the path documented in the [history format reference](https://github.com/manoelpanev/OpenWizardAI/blob/main/docs/openwizardai-cue-advanced.md).
...
- **No dashboard.** Trends live in the analyst's weekly report. OpenWizardAI's Document Graph can help you navigate proposals + evaluations via `[[wiki]]` links if you author the markdown that way.
...
- [Case Study: OpenWizardAI Marketing Pipeline](/openwizardai-cue-marketing-example) - what this pattern looks like in production, with eight chains driving the @manoelpanevAI X account.
- [Cue Configuration](/openwizardai-cue-configuration) - full subscription schema.
- [Cue Advanced Patterns](/openwizardai-cue-advanced) - fan-in, fan-out, command nodes, template variables.
- [Cue Examples](/openwizardai-cue-examples) - copy-paste-ready pipelines for common workflows.
