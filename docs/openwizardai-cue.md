...
OpenWizardAI Cue is an event-driven automation engine that watches for things happening in your projects and automatically sends prompts to your agents in response. Instead of manually kicking off tasks, you define **subscriptions** - trigger-prompt pairings - in a YAML file, and Cue handles the rest.

<Note>
OpenWizardAI Cue is an **Encore Feature** - it's on by default. Turn it off in **Settings > Encore Features** to hide the shortcut, modal, and automation engine.
</Note>
...
- **Trigger from the CLI** - run `openwizardai-cli cue trigger` to fire a subscription on demand from scripts, CI/CD, or other agents

## Turning Cue On and Off

Cue is on out of the box. OpenWizardAI automatically scans all your active agents for `.openwizardai/cue.yaml` files in their project roots, and the Cue engine starts as soon as it finds one - no restart required. An agent with no `.openwizardai/cue.yaml` runs nothing.

To turn Cue off entirely:

1. Open **Settings** (`Cmd+,` / `Ctrl+,`)
2. Navigate to the **Encore Features** tab
3. Toggle **OpenWizardAI Cue** off

## Quick Start

Create a file called `.openwizardai/cue.yaml` in your project (inside the `.openwizardai/` directory at the project root):
...
- Press `Cmd+K` / `Ctrl+K` and search for "OpenWizardAI Cue"
...
**From an agent:** ask any OpenWizardAI agent to open it and it will run `openwizardai-cli open cue`, optionally on a tab (`openwizardai-cli open cue --tab scheduled`). See [Opening a surface from the CLI](./cli-reference#openwizardai-cli-open).
...
Tasks created here and tasks created with `openwizardai-cli cue schedule` are the same objects in the same file - both surfaces read and write through one module, so the list is always the whole truth. That also means an agent can schedule work for you and you can see, re-time, or cancel it here:
...
- **Prompt files** under `.openwizardai/prompts/` are keyed by subscription name, so their paths stay valid.
...
The header **?** button opens a built-in quick-reference guide covering Cue's purpose, getting started, the full event type list, template variables, and advanced patterns. Use it as an in-app cheat sheet; for the full schema and examples, see [Configuration Reference](./openwizardai-cue-configuration), [Event Types](./openwizardai-cue-events), and [Advanced Patterns](./openwizardai-cue-advanced).

![Cue Help](./screenshots/cue-help.png)

## Configuration File

Cue is configured via a `.openwizardai/cue.yaml` file placed inside the `.openwizardai/` directory at your project root. Each agent has its **own** cue.yaml under its **own** project root - the engine reads only that file (no parent-directory walk, no shared workspace file). For pipelines that span agents at different roots, see [Multi-root pipelines](./openwizardai-cue-configuration#multi-root-pipelines-agents-in-different-project-roots) in the Configuration Reference. See the [Configuration Reference](./openwizardai-cue-configuration) for the complete YAML schema.
...
| `cli.trigger`         | Manual trigger via `openwizardai-cli` | -                                 |

See [Event Types](./openwizardai-cue-events) for detailed documentation and examples for each type.
...
See [Advanced Patterns](./openwizardai-cue-advanced) for the complete template variable reference.

## Advanced Features

Cue supports sophisticated automation patterns beyond simple trigger-prompt pairings:

- **[Fan-out](./openwizardai-cue-advanced#fan-out)** - One trigger fires against multiple target agents simultaneously
- **[Fan-in](./openwizardai-cue-advanced#fan-in)** - Wait for multiple agents to complete before triggering
- **[Payload filtering](./openwizardai-cue-advanced#filtering)** - Conditionally trigger based on event data (glob matching, comparisons, negation)
- **[Agent chaining](./openwizardai-cue-advanced#agent-chaining)** - Build multi-step pipelines where each agent's output feeds the next
- **[Concurrency control](./openwizardai-cue-advanced#concurrency-control)** - Limit simultaneous runs and queue overflow events

See [Advanced Patterns](./openwizardai-cue-advanced) for full documentation.
...
- **CLI triggers** - `cli.trigger` events require `openwizardai-cli` to be installed. See the [CLI documentation](./cli#cue-automation) for setup.
...
- **Hot reload** - The engine watches `.openwizardai/cue.yaml` for changes and reloads automatically - no need to restart OpenWizardAI
