---
title: Release Notes
description: Version history and changelog for OpenWizardAI releases
---

# Release Notes

This page documents the version history of OpenWizardAI, including new features, improvements, and bug fixes for each release.

<Tip>
OpenWizardAI can update itself automatically! This feature was introduced in **v0.8.7** (December 16, 2025). Enable auto-updates in Settings to stay current.
</Tip>

---

## v0.17.x - OpenWizardAI Cue
...
🖍️ **Mark up a screenshot before you hand it over.** Draw, label, and now crop a pasted image inside OpenWizardAI, with the crop staying editable afterwards so you can widen it back out. Ask an agent to save the result and it lands in your project.
...
- 🧰 **A per-agent Setup Script runs inside every worktree OpenWizardAI creates**, so your `.env.local` and your installed dependencies are already there before you start work.
- 🔌 **Switch an environment variable off without deleting it.** An eye button parks any variable: it keeps its key and value and stays editable, but nothing OpenWizardAI runs can see it. Test without a proxy or an API key and put it back in one click, per agent, worktrees included.
- 🔤 **An environment variable name suggests itself.** Typing `CLAUDE_HOME` when the variable is really `CLAUDE_CONFIG_DIR` sets something the CLI ignores, and the agent then runs as if you had configured nothing at all. The name field completes as you type now, from a short per-provider list of the variables people actually reach for (account directory, credentials, gateway, model) plus every name you have already set anywhere in OpenWizardAI. Only names are remembered, never values, so a recalled `ANTHROPIC_API_KEY` is a spelling and nothing more, and anything outside the list can still be typed by hand.
...
- 🖥️ **`openwizardai-cli` runs the binary you pointed it at.** Every CLI spawn fell back to the bare command name, so a custom agent path was ignored and the desktop app and the CLI could run two different copies of the same agent. On Windows nothing ran at all. The CLI resolves the real binary now, you can pick which installation to run when several are found, and installing `openwizardai-cli` on Windows actually puts it on your PATH.
...
- 🍎 **Let an agent reach your calendar, contacts, photos, and folders on macOS.** Anything an agent shelled out to was denied on the spot and in silence. OpenWizardAI now declares that whole surface, so the request reaches you as a normal macOS prompt and you decide service by service.
- 🔐 **A Group Chat participant you put on a remote host stays there.** If the SSH remote cannot be reached, OpenWizardAI says so and names it, rather than quietly running the work on your own machine against somebody else's paths.
- 🗃️ **Your groups survive a slow start.** When the group list could not be read at launch, OpenWizardAI saved the empty list over the real one and every launch after made the loss stick. One user lost 13 groups and the placement of 83 agents that way. A list OpenWizardAI never managed to read is never written now.
...
- 🎚️ **Per-phase model and effort.** Drop a `OPENWIZARDAI:MODEL` marker above a group of tasks and cheap survey work runs small while the hard design work runs at your provider's ceiling. Markers carry a reason now, shown behind an info button, so a playbook you read back a week later can be audited instead of merely obeyed. `openwizardai-cli` speaks the same directives.
...
- 🛡️ **Cue can screen GitHub issues for prompt injection.** An issue body is the one Cue input a stranger can write, and it goes straight to an agent. OpenWizardAI can score that text, refuse anything that reads as an attempt to steer your agent, and show you what it blocked. Off until you supply a 0DIN token.
...
- 🗜️ **A trigger that fires a thousand times reads as one History row.** A busy heartbeat could bury everything else under hundreds of near-identical entries. Repeated runs collapse into one row with a tally ("1,382 runs - 3 failed"), a clean run that printed nothing leaves no row at all, and runs from before OpenWizardAI recorded output stop drawing twice. Choose how long Cue keeps its history, from 7 days to a year.
...
- 🧩 **Long prompts reach Claude whole on background runs.** Typing a long prompt in one go could silently lose kilobyte-sized pieces of it, so Cue runs went ahead on prompts with chunks missing and nothing said so. OpenWizardAI feeds the prompt in pieces, waits on Claude's reads rather than a fixed clock, and fails loudly if what arrived is not what was sent.
- 🍺 **Cue finds the tools you installed.** An OpenWizardAI opened from the Dock inherits a bare system PATH with no Homebrew on it, so anything in `/opt/homebrew/bin` came back "command not found". Cue shell commands, agent runs, and worktree setup scripts all get the full PATH now.
...
- 🫥 **A resumed conversation stops opening with OpenWizardAI's own system prompt at the top of it.** A tab hydrated from disk drew the whole system envelope as if you had typed it, burying your real prompt under the conductor profile and a couple of file paths.
...
- ⏹️ **Stop stops the whole agent**, not just the tab you are looking at, and OpenWizardAI no longer calls itself idle while a dozen messages are still queued behind a finished turn.
...
- 🎧 **CSV, TSV, audio, and video get real support.** Tabular previews get a keyboard-driven row detail view, and media plays inside OpenWizardAI with a 0.25x to 4x speed control that survives a restart.
- ▶️ **A media player that follows you around.** Browse away and it detaches into a floating widget you can drag, resize, minimize to a play/pause pill, or double-click to re-dock. It holds a queue with each file's running time and what is left, and a media link in a chat plays in OpenWizardAI instead of launching your OS default app. `Opt+Cmd+M` opens it.
...
- 📝 **OpenWizardAI Prompts is a real editor now**, riding the same stack the Memory Viewer uses.
...
- ⌨️ **A shortcut you try to bind over an existing one is refused**, naming the action that holds it, instead of silently stealing the key and leaving the older action dead. OpenWizardAI also will not take the combinations the operating system owns, like `Cmd+Shift+Arrow` for extending a text selection.
...
- ✨ **Surface Gloss** adds a light source to OpenWizardAI's chrome so the title bar, sidebars, tab strip, and composer read as stacked layers instead of one flat sheet of paint. Four stops in Settings, off by default.
- 🎨 **More of OpenWizardAI follows your theme**, including the connecting orange, the context-window warning sash, and the pulse and glow effects that used to fall back to indigo on all twenty themes. A theme change from `openwizardai-cli` now shows up immediately instead of waiting for a relaunch.
...
- 🛟 **See how much downtime OpenWizardAI absorbed for you.** A Resilience section keeps score of the outages your agents walked into and came back from, how much waiting OpenWizardAI sat through on your behalf, and how many retries it took.
...
- ⏲️ **WakaTime credits the work your agents did without you**, counting Cue runs and `openwizardai-cli` time, and stops mislabeling work done over SSH.
...
- 🛌 **Sleep prevention no longer freezes your Mac's housekeeping.** OpenWizardAI was asking macOS to keep the display awake, which is how the system decides someone is sitting there, so Photos clustering, Spotlight indexing, and Time Machine were all suppressed for as long as OpenWizardAI was open.
...
🌐 **There is also a new github.com/manoelpanev/OpenWizardAI.** It is not part of this download and it did not ship with the app, it just went live. Go have a look, and tell us what you think.

### Previous Releases in this Series

- **v0.17.3** (July 4, 2026) - OpenWizardAI Cue
- **v0.17.2** (June 27, 2026) - OpenWizardAI Cue
- **v0.17.1** (June 20, 2026) - OpenWizardAI Cue
- **v0.17.0** (June 15, 2026) - OpenWizardAI Cue

---

## v0.15.x - OpenWizardAI Symphony

**Latest: v0.15.3** | Released April 5, 2026

# Major 0.15.x Additions

🎶 **OpenWizardAI Symphony** — Contribute to open source with AI assistance! Browse curated issues from projects with the `github.com/manoelpanev/OpenWizardAI` label, clone repos with one click, and automatically process the relevant Auto Run playbooks. Track your contributions, streaks, and stats. You're contributing CPU and tokens towards your favorite open source projects and features.
...
🏷️ **Conductor Profile** — Available under Settings > General. Provide a short description on how OpenWizardAI agents should interface with you.
...
- **CLI settings management:** Full `openwizardai-cli settings` command suite — list, get, set, and reset any OpenWizardAI setting from the command line. Includes per-agent configuration (custom paths, args, env vars, model overrides). Supports category filtering, verbose descriptions, and machine-readable JSON output for scripting
...
- **v0.15.2** (March 12, 2026) - OpenWizardAI Symphony
- **v0.15.1** (March 3, 2026) - OpenWizardAI Symphony
...
- Desktop app performance improvements (more to come on this, we want OpenWizardAI blazing fast) 🐌
...
- [OpenWizardAI-Playbooks](https://github.com/pedramamini/OpenWizardAI-Playbooks) can now contain non-markdown assets 📙
...
- Added a Playbook Exchange for downloading pre-defined Auto Run playbooks from [OpenWizardAI-Playbooks](https://github.com/pedramamini/OpenWizardAI-Playbooks) 📕
...
🎯 Added [GitHub Spec-Kit](https://github.com/github/spec-kit) commands into OpenWizardAI with a built in updater to grab the latest prompts from the repository. We do override `/speckit-implement` (the final step) to create Auto Run docs and guide the user through their execution, which thanks to Wortrees from v0.11.x allows us to run in parallel!
...
- **v0.8.2** (December 14, 2025) - manoelpanev.ai Leaderboard
- **v0.8.1** (December 13, 2025) - manoelpanev.ai Leaderboard (Signed!)
- **v0.8.0** (December 12, 2025) - manoelpanev.ai Leaderboard
...
- **v0.7.0** (December 7, 2025) - OpenWizardAI CLI
...
- Added "Made with OpenWizardAI" badge to README header 🎯
...
All releases are available on the [GitHub Releases page](https://github.com/manoelpanev/OpenWizardAI/releases).

OpenWizardAI is available for:
