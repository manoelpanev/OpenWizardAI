---
title: Cue Examples
description: Real-world OpenWizardAI Cue configurations for common automation workflows.
icon: lightbulb
---

Complete, copy-paste-ready `.openwizardai/cue.yaml` configurations for common workflows. Each example is self-contained - drop it into your project's `.openwizardai/` directory and adjust agent names to match your Left Bar.
...
Run setup tasks once when the OpenWizardAI application launches - install dependencies, verify environment, run health checks.
...
<Note>
This example assumes each agent has its own project root and therefore its own `.openwizardai/cue.yaml`. The three files below live under three different project roots - that's the supported pattern for multi-root pipelines (the engine reads each agent's own cue.yaml and never aggregates across roots). See [Multi-root pipelines](./openwizardai-cue-configuration#multi-root-pipelines-agents-in-different-project-roots) for the full rule. If all three agents share one project root, put all three subscriptions in a single `.openwizardai/cue.yaml` and use `agent_id` (or `settings.owner_agent_id` plus an explicit `agent_id` per sub) to route each one.
</Note>

The `linter` agent's `.openwizardai/cue.yaml`:
...
The `tester` agent's `.openwizardai/cue.yaml`:
...
The `deployer` agent's `.openwizardai/cue.yaml`:
...
The `worker` agent's `.openwizardai/cue.yaml`:
...
The `reviewer` agent's `.openwizardai/cue.yaml`:
...
The `coordinator` agent's `.openwizardai/cue.yaml`:
...
The `pr-reviewer` agent's `.openwizardai/cue.yaml`:
...
The `security-reviewer` agent's `.openwizardai/cue.yaml`:
...
The `builder` agent's `.openwizardai/cue.yaml`:
...
The `judge` agent's `.openwizardai/cue.yaml`:
...
The `reporter` agent's `.openwizardai/cue.yaml`:
...
The `summarizer` agent's `.openwizardai/cue.yaml`:
...
openwizardai-cli cue trigger code-review

# Review with specific focus
openwizardai-cli cue trigger code-review --prompt "Focus on the auth module changes"
```
...
```bash
# From a CI pipeline
openwizardai-cli cue trigger deploy --prompt "staging" --json

# From a release script
openwizardai-cli cue trigger deploy --prompt "production"
```
...
Schedule a single action tied to a wall-clock moment - "in 20 minutes do X", "tomorrow at 9am email me a summary", "remind me at 4pm to push the rc branch". These are authored exclusively through `openwizardai-cli cue schedule` (the CLI handles ISO formatting, timezone offsets, and writes directly to the owning agent's `.openwizardai/cue.yaml`). The subscriptions self-destruct from the YAML after they fire.

See [time.once event reference](./openwizardai-cue-events#time-once) for the full schema.
...
Resulting subscription (the CLI writes this into the agent's `.openwizardai/cue.yaml`):
