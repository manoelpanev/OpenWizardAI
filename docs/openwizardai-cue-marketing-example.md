---
title: 'Case Study: OpenWizardAI Marketing Pipeline'
description: How @manoelpanevAI's eight-chain Cue pipeline runs a self-tuning Twitter marketing loop in production.
icon: megaphone
---

This is a real production pipeline - the one driving the [@manoelpanevAI](https://x.com/manoelpanevAI) X account. It's also a working implementation of the [Karpathy Loop case study](/openwizardai-cue-karpathy-loop), built before that doc was written, in a domain (marketing) where the feedback signal is external (audience CTR) instead of synthetic (an evaluator agent's score).
...
Stick with the [three-chain Karpathy Loop](/openwizardai-cue-karpathy-loop) when:
...
- [Case Study: The Karpathy Loop](/openwizardai-cue-karpathy-loop) - the abstract pattern this pipeline is an instance of.
- [Cue Configuration](/openwizardai-cue-configuration) - full subscription schema.
- [Cue Advanced Patterns](/openwizardai-cue-advanced) - fan-in, fan-out, command nodes.
- [@manoelpanevAI](https://x.com/manoelpanevAI) - the account this pipeline drives.
