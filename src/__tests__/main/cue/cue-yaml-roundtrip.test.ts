...
describe('cue YAML roundtrip', () => {
	...
	it('write creates .openwizardai/ directory if missing', () => {
		const openwizardaiDir = path.join(projectRoot, '.openwizardai');
		expect(fs.existsSync(openwizardaiDir)).toBe(false);

		writeCueConfigFile(projectRoot, 'subscriptions: []');

		expect(fs.existsSync(openwizardaiDir)).toBe(true);
		expect(fs.existsSync(path.join(openwizardaiDir, 'cue.yaml'))).toBe(true);
	});
	...
	it('resolveCueConfigPath prefers canonical over legacy when both exist', () => {
		const legacyPath = path.join(projectRoot, 'openwizardai-cue.yaml');
		const canonicalDir = path.join(projectRoot, '.openwizardai');
...
describe('cue prompt-file roundtrip', () => {
	it('writes a .md prompt file and reads it back', () => {
		const rel = '.openwizardai/prompts/sub-1.md';
		writeCuePromptFile(projectRoot, rel, 'body content');
		const full = path.join(projectRoot, rel);
		expect(fs.readFileSync(full, 'utf-8')).toBe('body content');
	});

	it('creates nested subdirectories under .openwizardai/prompts/', () => {
		writeCuePromptFile(projectRoot, '.openwizardai/prompts/nested/deep/sub.md', 'body');
		expect(fs.existsSync(path.join(projectRoot, '.openwizardai/prompts/nested/deep/sub.md'))).toBe(
			true
		);
	});
	...
	it('rejects paths that escape the prompts directory', () => {
		expect(() =>
			writeCuePromptFile(projectRoot, '.openwizardai/prompts/../../escape.md', 'evil')
		).toThrow(/resolves outside the prompts directory/);
	});

	it('rejects non-.md files', () => {
		expect(() =>
			writeCuePromptFile(projectRoot, '.openwizardai/prompts/payload.sh', 'evil')
...
describe('orphan prompt file pruning', () => {
	it('removes .md files not in the keep-set', () => {
		writeCuePromptFile(projectRoot, '.openwizardai/prompts/keep.md', 'keep');
		writeCuePromptFile(projectRoot, '.openwizardai/prompts/drop.md', 'drop');

		const removed = pruneOrphanedPromptFiles(projectRoot, ['.openwizardai/prompts/keep.md']);

		expect(removed).toHaveLength(1);
		expect(removed[0]).toContain('drop.md');
		expect(fs.existsSync(path.join(projectRoot, '.openwizardai/prompts/keep.md'))).toBe(true);
		expect(fs.existsSync(path.join(projectRoot, '.openwizardai/prompts/drop.md'))).toBe(false);
	});
	...
	it('preserves non-.md files (only markdown is managed)', () => {
		...
		writeCuePromptFile(projectRoot, '.openwizardai/prompts/referenced.md', 'x');
		const extraPath = path.join(projectRoot, '.openwizardai/prompts/other.txt');
		fs.writeFileSync(extraPath, 'keep me', 'utf-8');

		const removed = pruneOrphanedPromptFiles(projectRoot, ['.openwizardai/prompts/referenced.md']);
	...
	it('handles nested subdirectories under prompts/', () => {
		writeCuePromptFile(projectRoot, '.openwizardai/prompts/a/keep.md', 'keep');
		writeCuePromptFile(projectRoot, '.openwizardai/prompts/b/drop.md', 'drop');

		const removed = pruneOrphanedPromptFiles(projectRoot, ['.openwizardai/prompts/a/keep.md']);
		expect(removed).toHaveLength(1);
	...
	it('skips absolute paths in the keep-set', () => {
		...
		writeCuePromptFile(projectRoot, '.openwizardai/prompts/one.md', 'x');
...
describe('yaml + prompt-file full roundtrip', () => {
	it('write YAML + prompt files → read back → same data', () => {
		const yaml = [
			...
			'    prompt_file: .openwizardai/prompts/morning.md',
			'  - name: output-prompt',
			'    event: agent.completed',
			'    prompt_file: .openwizardai/prompts/output.md',
			'    output_prompt_file: .openwizardai/prompts/output-phase2.md',
			'',
		].join('\n');

		writeCueConfigFile(projectRoot, yaml);
		writeCuePromptFile(projectRoot, '.openwizardai/prompts/morning.md', 'morning body');
		writeCuePromptFile(projectRoot, '.openwizardai/prompts/output.md', 'output body');
		writeCuePromptFile(projectRoot, '.openwizardai/prompts/output-phase2.md', 'phase-2 body');
		...
		// All prompts on disk
		expect(
			fs.readFileSync(path.join(projectRoot, '.openwizardai/prompts/morning.md'), 'utf-8')
		).toBe('morning body');
		expect(
			fs.readFileSync(path.join(projectRoot, '.openwizardai/prompts/output.md'), 'utf-8')
		).toBe('output body');
		expect(
			fs.readFileSync(path.join(projectRoot, '.openwizardai/prompts/output-phase2.md'), 'utf-8')
		).toBe('phase-2 body');
		...
		const removed = pruneOrphanedPromptFiles(projectRoot, [
			'.openwizardai/prompts/morning.md',
			'.openwizardai/prompts/output.md',
		]);
		expect(removed).toHaveLength(1);
		expect(removed[0]).toContain('output-phase2.md');
		expect(fs.existsSync(path.join(projectRoot, '.openwizardai/prompts/output-phase2.md'))).toBe(
			false
		);
	});

	it('overwriting YAML+prompts preserves the rest of .openwizardai/', () => {
		// User may have unrelated files (e.g. director-notes, other tooling)
		// in .openwizardai/ - we must not blow them away.
		const openwizardaiDir = path.join(projectRoot, '.openwizardai');
		fs.mkdirSync(openwizardaiDir, { recursive: true });
		fs.writeFileSync(path.join(openwizardaiDir, 'director-notes.md'), 'notes', 'utf-8');

		writeCueConfigFile(projectRoot, 'subscriptions: []');
		writeCuePromptFile(projectRoot, '.openwizardai/prompts/x.md', 'body');

		expect(fs.readFileSync(path.join(openwizardaiDir, 'director-notes.md'), 'utf-8')).toBe('notes');
	});
});
