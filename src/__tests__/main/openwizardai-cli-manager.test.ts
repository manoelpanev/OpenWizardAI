...
import { OpenWizardAICliManager, normalizeVersion } from '../../main/openwizardai-cli-manager';

describe('OpenWizardAICliManager', () => {
	beforeEach(() => {
		vi.clearAllMocks();
	...
	describe('normalizeVersion', () => {
		it('finds a version line after incidental logger output', () => {
			expect(
				normalizeVersion(
					'[Logger] File logging enabled: C:\\logs\\openwizardai-debug-2026-09-10.log\r\n0.17.4\r\n'
				)
			).toBe('0.17.4');
		});
	...
	it('updates the Windows user PATH with PowerShell directly and valid block syntax', async () => {
		const manager = new OpenWizardAICliManager();
