...
const buildSshCommand = vi
	.fn()
	.mockResolvedValue({ command: 'ssh', args: ['host', 'openwizardai-p --version'] });
...
} from '../../../main/agents/probeRemoteOpenWizardAIP';
import {
...
describe('probeRemoteOpenWizardAIP', () => {
	beforeEach(() => {
		__clearRemoteOpenWizardAIPCache();
	...
	it('launch-tests `openwizardai-p --version` rather than checking the path', async () => {
		execFileNoThrow.mockResolvedValue({ exitCode: 0, stdout: '0.16.20-RC\n', stderr: '' });
		await probeRemoteOpenWizardAIP(remote);
		// The probe must actually run openwizardai-p, not just `command -v` it.
		expect(buildSshCommand).toHaveBeenCalledWith(
			remote,
			expect.objectContaining({ command: 'openwizardai-p', args: ['--version'] })
		);
	});

	it('caches true when openwizardai-p launches (exit 0 with a version line)', async () => {
		execFileNoThrow.mockResolvedValue({ exitCode: 0, stdout: '0.16.20-RC\n', stderr: '' });
		const r = await probeRemoteOpenWizardAIP(remote);
		expect(r).toBe(true);
		expect(getRemoteOpenWizardAIPAvailable('remote-1')).toBe(true);
	});

	it('caches false when openwizardai-p is not installed (exit 127)', async () => {
		execFileNoThrow.mockResolvedValue({
			exitCode: 127,
			stdout: '',
			stderr: 'bash: openwizardai-p: command not found',
		});
		const r = await probeRemoteOpenWizardAIP(remote);
		expect(r).toBe(false);
		expect(getRemoteOpenWizardAIPAvailable('remote-1')).toBe(false);
	});

	it('caches false when openwizardai-p exists but cannot launch (node/node-pty missing)', async () => {
...
describe('ensureRemoteOpenWizardAIPProbed', () => {
	beforeEach(() => {
		__clearRemoteOpenWizardAIPCache();
		execFileNoThrow.mockReset();
	});
	...
	it('probes once on a cold cache, then serves the cached result without re-probing', async () => {
		...
		const first = await ensureRemoteOpenWizardAIPProbed(remote);
		expect(first).toBe(false);
		expect(execFileNoThrow).toHaveBeenCalledTimes(1);

		// Fresh cache: no second probe.
		const second = await ensureRemoteOpenWizardAIPProbed(remote);
