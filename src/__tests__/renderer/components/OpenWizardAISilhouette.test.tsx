/**
 * Tests for OpenWizardAISilhouette component
 *
 * OpenWizardAISilhouette is a conductor silhouette image component that:
 * - Displays a PNG image of a conductor
 * - Supports dark/light variants for different backgrounds
 * - Has configurable size
 * - Also exports AnimatedOpenWizardAI with conducting motion animation
 */

import React from 'react';
import { describe, it, expect, vi, beforeEach, afterEach } from 'vitest';
import { render, screen } from '@testing-library/react';
import OpenWizardAISilhouette, {
	OpenWizardAISilhouette as NamedOpenWizardAISilhouette,
	AnimatedOpenWizardAI,
} from '../../../renderer/components/OpenWizardAISilhouette';
...
describe('OpenWizardAISilhouette', () => {
	describe('default export', () => {
		it('matches named export', () => {
			expect(OpenWizardAISilhouette).toBe(NamedOpenWizardAISilhouette);
		});

		it('renders with default props', () => {
			render(<OpenWizardAISilhouette />);

			const img = screen.getByRole('img');
			expect(img).toBeInTheDocument();
			expect(img).toHaveAttribute('alt', 'OpenWizardAI conductor silhouette');
		});
	});
	...
	describe('rendering', () => {
		...
		it('uses light variant when specified', () => {
			render(<OpenWizardAISilhouette variant="light" />);
		...
		it('uses dark variant when explicitly specified', () => {
			render(<OpenWizardAISilhouette variant="dark" />);
	...
	describe('size', () => {
		...
		it('accepts custom size', () => {
			render(<OpenWizardAISilhouette size={300} />);
		...
		it('handles small sizes', () => {
			render(<OpenWizardAISilhouette size={50} />);
		...
		it('handles large sizes', () => {
			render(<OpenWizardAISilhouette size={1000} />);
	...
	describe('className', () => {
		...
		it('accepts custom className', () => {
			render(<OpenWizardAISilhouette className="my-custom-class" />);
		...
		it('accepts multiple classes', () => {
			render(<OpenWizardAISilhouette className="class1 class2 class3" />);
	...
	describe('style', () => {
		...
		it('accepts custom style', () => {
			render(<OpenWizardAISilhouette style={{ opacity: 0.5 }} />);
		...
		it('merges custom styles with defaults', () => {
			render(<OpenWizardAISilhouette style={{ border: '1px solid red' }} />);
		...
		it('custom styles can override size', () => {
			render(<OpenWizardAISilhouette size={200} style={{ width: '100%' }} />);
	...
	describe('accessibility', () => {
		it('has alt text', () => {
			...
			const img = screen.getByAltText('OpenWizardAI conductor silhouette');
			expect(img).toBeInTheDocument();
		});
	});
});

describe('AnimatedOpenWizardAI', () => {
	describe('rendering', () => {
		it('renders an img element', () => {
			render(<AnimatedOpenWizardAI />);
		...
		it('has correct alt text', () => {
			...
			const img = screen.getByAltText('Animated openwizardai conductor');
			expect(img).toBeInTheDocument();
		});
		...
		it('uses light variant when specified', () => {
			render(<AnimatedOpenWizardAI variant="light" />);
	...
	describe('animation', () => {
		...
		it('animation is applied with custom styles', () => {
			render(<AnimatedOpenWizardAI style={{ opacity: 0.8 }} />);
	...
	describe('size', () => {
		...
		it('accepts custom size', () => {
			render(<AnimatedOpenWizardAI size={150} />);
	...
	describe('className', () => {
		it('accepts custom className', () => {
			render(<AnimatedOpenWizardAI className="animated-class" />);
	...
	describe('style', () => {
		...
		it('merges custom styles', () => {
			render(<AnimatedOpenWizardAI style={{ transform: 'scale(1.1)' }} />);
...
describe('CSS animation injection', () => {
	it('adds animation stylesheet to document head', () => {
		...
		const styleEl = document.getElementById('openwizardai-animation-styles');
	...
	it('only injects styles once', () => {
		...
		const styleElements = document.querySelectorAll('#openwizardai-animation-styles');
...
describe('edge cases', () => {
	it('handles zero size', () => {
		render(<OpenWizardAISilhouette size={0} />);
	...
	it('handles fractional size', () => {
		render(<OpenWizardAISilhouette size={150.5} />);
	...
	it('handles empty className', () => {
		render(<OpenWizardAISilhouette className="" />);
	...
	it('handles empty style object', () => {
		render(<OpenWizardAISilhouette style={{}} />);
	...
	it('renders both components simultaneously', () => {
		...
		expect(images[0]).toHaveAttribute('alt', 'OpenWizardAI conductor silhouette');
		expect(images[1]).toHaveAttribute('alt', 'Animated openwizardai conductor');
	});

	it('renders multiple instances with different props', () => {
		render(
			<>
				<OpenWizardAISilhouette variant="dark" size={100} />
				<OpenWizardAISilhouette variant="light" size={200} />
				<AnimatedOpenWizardAI variant="dark" size={300} />
			</>
		);
