...
function renderDocument(content: string) {
	...
	return render(
		<ReactMarkdown remarkPlugins={[remarkOpenWizardAIMarkers]} components={components}>
			{content}
		</ReactMarkdown>
	);
}
...
describe('marker pills on a document surface', () => {
	...
	it('shows a satisfied gate differently from a live one', () => {
		...
		renderDocument(
			['<!-- OPENWIZARDAI:HITL reason="Add the key" -->', '', '- [x] Bill the customer'].join('\n')
		);
		const pill = screen.getByTestId('openwizardai-marker-hitl');
	...
	it('shows a halt marker, which is why a run refuses to start', () => {
		renderDocument(
			['- [x] Done', '', '<!-- openwizardai:halt: the build is broken -->'].join('\n')
		);
		const pill = screen.getByTestId('openwizardai-marker-halt');
		expect(pill).toHaveAttribute('data-marker-status', 'live');
	...
	it('shows a document-scoped model hint', () => {
		renderDocument(
			['<!-- OPENWIZARDAI:MODEL tier="high" effort="high" -->', '', '- [ ] Design'].join('\n')
		);
		const pill = screen.getByTestId('openwizardai-marker-model');
	...
	it('shows an inline model hint beside its own task', () => {
		renderDocument('- [ ] Design the migration <!-- OPENWIZARDAI:MODEL tier="high" -->');
		const pill = screen.getByTestId('openwizardai-marker-model');
		expect(pill).toHaveAttribute('data-marker-status', 'live');
	...
	it('offers a peek at the reason without spending a line on it', () => {
		...
		renderDocument(
			[
				`<!-- OPENWIZARDAI:MODEL tier="high" effort="high" reason="${reason}" -->`,
				'',
				'- [ ] Design',
			].join('\n')
		);
		...
		expect(screen.getByTestId('openwizardai-marker-reason')).toBeInTheDocument();
	...
	it('draws no reason affordance when the marker carries no justification', () => {
		renderDocument('<!-- OPENWIZARDAI:MODEL tier="high" -->\n\n- [ ] Design');
		expect(screen.queryByTestId('openwizardai-marker-reason')).toBeNull();
	});
	...
	it('mutes a hint governing a later phase instead of retiring it', () => {
		...
		const [next, later] = screen.getAllByTestId('openwizardai-marker-model');
	...
	it('flags a misspelled attribute instead of rendering it as a real setting', () => {
		renderDocument('<!-- OPENWIZARDAI:MODEL tier="hgih" -->\n\n- [ ] Task');
		const pill = screen.getByTestId('openwizardai-marker-model');
	...
	it('drops the pill radius once the detail is long enough to wrap', () => {
		...
		renderDocument(
			[`<!-- OPENWIZARDAI:HITL reason="${reason}" -->`, '', '- [ ] Verify the chips'].join('\n')
		);
		const pill = screen.getByTestId('openwizardai-marker-hitl');
	...
	it('keeps the pill shape for a one-line chip', () => {
		renderDocument('- [ ] Design the migration <!-- OPENWIZARDAI:MODEL tier="high" -->');
		expect(screen.getByTestId('openwizardai-marker-model').style.borderRadius).toBe('999px');
	});
	...
	it('renders nothing for a marker inside a fenced code block', () => {
		...
		expect(screen.queryByTestId('openwizardai-marker-hitl')).toBeNull();
	});

	it('leaves an ordinary HTML comment alone', () => {
		const { container } = renderDocument('<!-- just a note -->\n\n- [ ] Task');
		expect(container.querySelector('[data-openwizardai-marker]')).toBeNull();
	});
	...
	it('does not render pills on the chat surface', () => {
		...
		const { container } = render(
			<ReactMarkdown components={components}>
				{'You can write <!-- OPENWIZARDAI:HITL reason="x" --> to pause a run.'}
			</ReactMarkdown>
		);
		expect(container.querySelector('[data-openwizardai-marker]')).toBeNull();
	});
