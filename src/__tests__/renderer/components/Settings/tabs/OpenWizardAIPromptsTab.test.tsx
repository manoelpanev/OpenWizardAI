/**
 * Tests for OpenWizardAIPromptsTab - selection precedence and persistence.
 *
 ...
 *   3) then the well-known openwizardai-system-prompt
...
import { OpenWizardAIPromptsTab } from '../../../../../renderer/components/Settings/tabs/OpenWizardAIPromptsTab';
...
const PROMPTS = [
	...
	},
	{
		...
		description: 'OpenWizardAI system context.',
		category: 'system',
		content: '# system',
		isModified: false,
	},
...
function setupWindowOpenWizardAI() {
...
describe('OpenWizardAIPromptsTab selection precedence', () => {
	beforeEach(() => {
		mockSetLastSelectedPromptId.mockReset();
		mockLastSelectedPromptId = null;
		setupWindowOpenWizardAI();
	});

	it('defaults to openwizardai-system-prompt when nothing else is specified', async () => {
		render(<OpenWizardAIPromptsTab theme={mockTheme} />);
		await waitFor(() => {
			expect(
				screen.getByRole('heading', { name: /^openwizardai-system-prompt/ })
			).toBeInTheDocument();
		});
	});
	...
	it('prefers an explicit initialSelectedPromptId over the remembered one', async () => {
		mockLastSelectedPromptId = 'wizard-system';
		render(<OpenWizardAIPromptsTab theme={mockTheme} initialSelectedPromptId="autorun-default" />);
		await waitFor(() => {
	...
	it('falls back to the first prompt if neither recall nor the default system prompt exist', async () => {
		...
		(window as any).openwizardai.prompts.getAll = vi.fn(async () => ({
			success: true,
			prompts: [
				{
					id: 'autorun-default',
	...
	it('persists lastSelectedPromptId on selection change', async () => {
		render(<OpenWizardAIPromptsTab theme={mockTheme} />);
		await waitFor(() => screen.getByRole('heading', { name: /^openwizardai-system-prompt/ }));
	...
	it('emits data-item-id on each list item so the shared list is scrollable into view', async () => {
		const { container } = render(<OpenWizardAIPromptsTab theme={mockTheme} />);
		await waitFor(() => screen.getByRole('heading', { name: /^openwizardai-system-prompt/ }));
	...
	it('keeps a prompt with unsaved edits in the list even when the filter excludes it', async () => {
		...
		expect(screen.getByRole('button', { name: /openwizardai-system-prompt/ })).toBeInTheDocument();
		expect(
			screen.getByRole('heading', { name: /^openwizardai-system-prompt/ })
		).toBeInTheDocument();
	});
