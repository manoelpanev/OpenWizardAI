/**
 * @file args.test.ts
 * @description Tests for src/openwizardai-p/args.ts argv parser.
 *
...
describe('parseArgs', () => {
	...
	describe('stripped flags', () => {
		...
		// Regression: when OpenWizardAI forwards its API-mode claude args verbatim
		// to a custom-path openwizardai-p (the opt-in route), the argv looks like
		...
		// Regression: OpenWizardAI's ChildProcessSpawner appends the prompt after a `--`
		...
		it('treats `--` as the end-of-options marker (full OpenWizardAI spawn line)', () => {
			const result = callArgs([
				'--print',
				'--verbose',
				'--output-format',
	...
	describe('--resume', () => {
		...
		it('coexists with --input-format stream-json (follow-up image turn)', () => {
			...
			// --input-format is consumed by openwizardai-p; only --resume reaches claude.
