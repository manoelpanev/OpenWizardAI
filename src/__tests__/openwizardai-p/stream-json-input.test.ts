 ...
 * @description Tests for src/openwizardai-p/stream-json-input.ts - translation of
 * OpenWizardAI's `--input-format stream-json` envelope into a TUI-ready prompt
...
} from '../../openwizardai-p/stream-json-input';
...
describe('cleanupStreamJsonImages', () => {
	it('unlinks each path and silently ignores already-missing files', () => {
		const tmp = path.join(os.tmpdir(), `openwizardai-p-cleanup-test-${process.pid}.txt`);
