 ...
 * @description Tests for src/openwizardai-p/timeout-report.ts stderr reports.
 *
...
import { formatScreenTailReport, idleTimeoutMessage } from '../../openwizardai-p/timeout-report';
...
describe('formatScreenTailReport', () => {
	it('writes the message, then the screen tail under the shared header', () => {
		...
		expect(report).toBe(
			'openwizardai-p: something stalled.\n' +
				'openwizardai-p: last screen at timeout (ANSI-stripped tail):\n' +
				'Do you want to proceed?\n❯ 1. Yes\n'
		);
	});
