 ...
 * @description Tests for src/openwizardai-p/session-watcher.ts - discovers a
...
import {
	...
	cwdSlug,
	discoverSessionId,
} from '../../openwizardai-p/session-watcher';
...
describe('session-watcher', () => {
	...
	beforeEach(() => {
		tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'openwizardai-p-session-watcher-'));
	...
	describe('cwdSlug()', () => {
		...
		// src/shared/pathUtils.ts, because the openwizardai-p bundle stays lean and
		...
		// doesn't, openwizardai-p and the desktop app look in different directories
