 ...
 * @description Tests for src/openwizardai-p/tui-driver.ts - the slim PTY driver
 * that surfaces 'ready' / 'limit-hit' / 'line' / 'exit' events to openwizardai-p's
...
import {
	...
	TuiDriver,
} from '../../openwizardai-p/tui-driver';
...
describe('TuiDriver', () => {
	...
	describe('acceptWorkspaceTrust (usage probe folder)', () => {
		...
		async function makeTrustingDriver(): Promise<TuiDriver> {
			const driver = new TuiDriver({
				binPath: 'claude',
				args: [],
				cwd: '/tmp/openwizardai-claude-usage-probe',
				env: {},
	...
	describe("'limit-hit' event (c)", () => {
		...
		// This is exactly what regressed when an agent deployed OpenWizardAI's own
		...
		it.each([
			...
			'When your usage limit is reached, OpenWizardAI falls back to API.',
			...
			"When you've hit your session limit, OpenWizardAI schedules a retry.",
