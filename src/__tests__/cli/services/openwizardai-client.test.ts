/**
 * @file openwizardai-client.test.ts
 * @description Tests for the CLI WebSocket client service
 *
 * Tests the OpenWizardAIClient class including:
 * - Connection lifecycle (connect, disconnect)
 * - Command sending with response matching
 * - Timeout handling
 * - withOpenWizardAIClient helper lifecycle
...
describe('OpenWizardAIClient', () => {
	...
	describe('connect()', () => {
		it('should throw when no discovery file exists', async () => {
			...
			await expect(client.connect()).rejects.toThrow('OpenWizardAI desktop app is not running');
		});
		...
		it('should throw when PID is stale', async () => {
			...
			const client = new OpenWizardAIClient();
			await expect(client.connect()).rejects.toThrow('OpenWizardAI discovery file is stale');
		});
		...
		it('should reject on WebSocket error', async () => {
			...
			await expect(connectPromise).rejects.toThrow(
				'Failed to connect to OpenWizardAI: Connection refused'
			);
		});
		...
		it('should timeout after 5 seconds', async () => {
			...
			await expect(connectPromise).rejects.toThrow('Connection to OpenWizardAI timed out');
		});
	});

	describe('sendCommand()', () => {
		async function createConnectedClient(): Promise<OpenWizardAIClient> {
			vi.mocked(readCliServerInfo).mockReturnValue({
				port: 3000,
				token: 'test-token',
				pid: 12345,
		...
		it('should use default 10s timeout', async () => {
			...
			await expect(commandPromise).rejects.toThrow('Timed out waiting for the OpenWizardAI app');
		});
...
describe('withOpenWizardAIClient()', () => {
	beforeEach(() => {
		vi.clearAllMocks();
		vi.useFakeTimers();
	});
	...
	it('should connect, run action, and disconnect', async () => {
		...
		const resultPromise = withOpenWizardAIClient(actionFn);
	...
	it('should propagate connection errors', async () => {
		...
		await expect(withOpenWizardAIClient(async () => 'should not reach')).rejects.toThrow(
