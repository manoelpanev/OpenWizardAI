...
export interface HistoryHandlerDependencies {
	 ...
	 * into `<project>/.openwizardai/history/history-<hostname>.jsonl` on the local
	 * filesystem so other OpenWizardAI instances reading the same project directory
			...
			async (projectPath?: string, sessionId?: string, sharedContext?: SharedHistoryContext) => {
				...
				try {
					if (sharedContext?.sshRemoteId && sharedContext?.remoteCwd) {
						...
						// Local session: read .openwizardai/history/ from project dir
	...
	ipcMain.handle(
		...
		withIpcErrorLogging(
			...
			async (
				...
				// has foreign-host JSONL files in `.openwizardai/history/`.
				// Typical of an agent running directly on the remote machine
				// with `shareHistoryToProjectDir` on so a paired OpenWizardAI
	...
	ipcMain.handle(
		...
		withIpcErrorLogging(
			...
			async (entry: HistoryEntry, sharedContext?: SharedHistoryContext) => {
				...
				// Shared history: write to remote .openwizardai/history/ (only for SSH sessions with syncHistory enabled)
				if (sharedContext?.sshRemoteId && sharedContext?.remoteCwd) {
					const sshRemote = deps.getSshRemoteById?.(sharedContext.sshRemoteId);
					if (sshRemote) {
				...
				// Shared history: also mirror to the local project's .openwizardai/history/
				...
				// signal that another OpenWizardAI instance (typically SSH'd into this
