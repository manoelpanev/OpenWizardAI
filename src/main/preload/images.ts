 ...
 * `openwizardai-image://store/<sha>.<ext>`. The desktop renderer loads those refs
 * directly via the openwizardai-image protocol (`<img src>`), so most UI needs
...
export function createImagesApi() {
	return {
		/**
		 * Resolve a `openwizardai-image://` reference to a data URL. Passes through
