...
const CLI_BINARY_NAME = 'openwizardai-cli';
const LOG_CONTEXT = 'OpenWizardAICliManager';
...
export class OpenWizardAICliManager {
	private readonly posixPathMarker = '# Added by OpenWizardAI CLI installer';
	...
	private getBundledCliCandidates(): string[] {
		return [
			path.join(process.resourcesPath, 'openwizardai-cli.js'),
			path.resolve(app.getAppPath(), 'dist', 'cli', 'openwizardai-cli.js'),
			path.resolve(__dirname, '..', 'cli', 'openwizardai-cli.js'),
		];
	}

	private async resolveBundledCliPath(): Promise<string | null> {
		const candidates = this.getBundledCliCandidates();
		logger.debug('Resolving bundled openwizardai-cli.js path', LOG_CONTEXT, { candidates });
		for (const candidate of candidates) {
			try {
				await fs.promises.access(candidate, fs.constants.R_OK);
				logger.info('Resolved bundled openwizardai-cli.js', LOG_CONTEXT, { path: candidate });
				return candidate;
			} catch (err) {
		...
		logger.warn('No bundled openwizardai-cli.js candidate was readable', LOG_CONTEXT, {
			candidates,
		});
		return null;
	}
	...
	private async detectCliPath(useExpandedEnv: boolean): Promise<string | null> {
		...
		if (whichResult.exitCode !== 0 || !whichResult.stdout.trim()) {
			logger.debug('openwizardai-cli not found on PATH', LOG_CONTEXT, {
				useExpandedEnv,
				whichCommand,
		...
		logger.debug('openwizardai-cli detected on PATH', LOG_CONTEXT, {
			useExpandedEnv,
			whichCommand,
			resolved,
		});
	...
	private async readCliVersion(commandPath: string): Promise<string | null> {
		...
		if (versionResult.exitCode !== 0 || !versionResult.stdout.trim()) {
			logger.warn('Failed to read openwizardai-cli version', LOG_CONTEXT, {
				commandPath,
		...
		logger.debug('Read openwizardai-cli version', LOG_CONTEXT, { commandPath, version });
		return version;
	}
	...
	private async writeUnixShim(installPath: string, bundledCliPath: string): Promise<void> {
		...
		logger.info('Writing Unix openwizardai-cli shim', LOG_CONTEXT, {
			installPath,
			bundledCliPath,
	...
	private async writeWindowsShim(installPath: string, bundledCliPath: string): Promise<void> {
		...
		logger.info('Writing Windows openwizardai-cli shim', LOG_CONTEXT, {
			installPath,
			bundledCliPath,
			runtimePath: process.execPath,
		});
	...
	private async ensurePosixPathExport(
		...
		logger.info('Ensuring POSIX PATH export for openwizardai-cli', LOG_CONTEXT, {
			installDir,
			shellName,
		...
		for (const rcFile of rcFiles) {
			...
			try {
				...
				logger.info('Appended openwizardai-cli PATH export to rc file', LOG_CONTEXT, { rcPath });
				updated = true;
	...
	private async ensureWindowsUserPath(installDir: string): Promise<boolean> {
		...
		if (result.exitCode !== 0) {
			logger.error('Failed to update Windows user PATH for openwizardai-cli', LOG_CONTEXT, {
	...
	async checkStatus(): Promise<OpenWizardAICliStatus> {
		...
		const status: OpenWizardAICliStatus = {
			expectedVersion,
			installed,
			inPath,
			inShellPath,
		...
		logger.info('Checked openwizardai-cli status', LOG_CONTEXT, {
			expectedVersion,
			installedVersion,
			installed,
			inPath,
	...
	async installOrUpdate(): Promise<OpenWizardAICliInstallResult> {
		const installDir = this.getInstallDir();
		const installPath = this.getInstallPath();

		logger.info('Starting openwizardai-cli install/update', LOG_CONTEXT, {
			installDir,
			installPath,
		...
		if (!bundledCliPath) {
			const candidates = this.getBundledCliCandidates();
			logger.error('Unable to locate bundled openwizardai-cli.js in app resources', LOG_CONTEXT, {
				candidates,
			});
			throw new Error(
				`Unable to locate bundled openwizardai-cli.js in app resources. Tried: ${candidates.join(', ')}`
			);
		}
		...
		try {
			...
			logger.info('openwizardai-cli shim written', LOG_CONTEXT, { installPath });
		} catch (err) {
			logger.error('Failed to write openwizardai-cli shim', LOG_CONTEXT, {
				installPath,
				bundledCliPath,
				error: err instanceof Error ? err.message : String(err),
			});
		...
		if (!alreadyInPath) {
			if (isWindows()) {
				...
				if (!pathUpdated) {
					pathUpdateError = 'Failed to update Windows user PATH for openwizardai-cli';
				}
			} else {
		...
		logger.info('openwizardai-cli install/update finished', LOG_CONTEXT, {
			success,
			executionSucceeded,
			shimVersion,
			pathUpdated,
		...
		if (!success) {
			logger.warn('openwizardai-cli install/update completed with issues', LOG_CONTEXT, {
