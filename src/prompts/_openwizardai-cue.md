...
**Cue** is OpenWizardAI's event-driven automation engine. A **subscription** listens for an event and fires a prompt at a target agent. Subscriptions are defined per-project in a YAML file.

### Configuration File

**Canonical path: `<project-root>/.openwizardai/cue.yaml`** - always write new configs here. The engine creates the `.openwizardai/` directory automatically on save.

Legacy path: `<project-root>/openwizardai-cue.yaml` (deprecated). The engine still reads it if present for backwards compatibility, but every save migrates to the canonical location, so do **not** write new files there.

When you need to find an existing config, check `.openwizardai/cue.yaml` first, then fall back to `openwizardai-cue.yaml` at the project root.
...
| `cli.trigger`         | Manually fired via `openwizardai-cli cue trigger`                    | -                                                                                                   |

### Scheduled Tasks (one-shot and repeating)

Every clock-driven subscription - `time.once`, `time.scheduled`, and `time.heartbeat` - is a **Scheduled Task**. They share one authoring surface (`openwizardai-cli cue schedule`) and one management surface in the app (**OpenWizardAI Cue → Scheduled Tasks**, reachable with `openwizardai-cli open cue --tab scheduled`), where the user can re-time, pause, resume, or cancel anything you created. Pick the event from how the user phrased the repetition:
...
```bash
{{OPENWIZARDAI_CLI_PATH}} cue schedule --list [--kind once|daily|interval] [--json]
{{OPENWIZARDAI_CLI_PATH}} cue schedule --reschedule <name> --daily-at 09:15   # timing flag must match the task's kind
{{OPENWIZARDAI_CLI_PATH}} cue schedule --pause <name>                          # stops firing, keeps the task
{{OPENWIZARDAI_CLI_PATH}} cue schedule --resume <name>
{{OPENWIZARDAI_CLI_PATH}} cue schedule --cancel <name>                         # deletes it
```
...
**Missed-fire grace window:** if OpenWizardAI is closed when `fire_at` arrives, the engine fires the subscription on next startup as long as the current time is within `grace_minutes` of `fire_at`. Default `grace_minutes` is `360` (6 hours). Past the grace window, the sub is logged as expired and self-destructs without firing - pick a smaller window for time-sensitive reminders, a larger one for anything you don't mind being late.

**Default pipeline:** subscriptions authored via `openwizardai-cli cue schedule` land in `pipeline_name: "Tasks"` so they group into a single "Tasks" card in the Cue dashboard. Override `--pipeline` only when the one-off naturally belongs to an existing pipeline.

**Authoring rule:** **never hand-write `time.once` subscriptions** - always go through `openwizardai-cli cue schedule`. The CLI handles ISO formatting, timezone offsets, target_node_key generation, and writes directly to the owning agent's `.openwizardai/cue.yaml`.
...
```bash
{{OPENWIZARDAI_CLI_PATH}} cue schedule \
  --in 20m \
  --agent {{AGENT_ID}} \
...
{{OPENWIZARDAI_CLI_PATH}} cue schedule --list

# Cancel a pending task by its sub name
{{OPENWIZARDAI_CLI_PATH}} cue schedule --cancel tasks-once-push-rc-reminder
```
...
- **Command node:** a subscription with `action: command` that runs a shell command (`command.mode: shell`) or a `openwizardai-cli` call (`command.mode: cli`) instead of an AI prompt. Emits `agent.completed` like any other run, so a downstream agent reads its stdout via `{{CUE_SOURCE_OUTPUT}}` (use `source_sub: <command-sub-name>` to pin the chain to that command and not other completions in the same session). See **Command Nodes** below for the full schema.
- **`cli_output`:** an object `cli_output: { target: "<source-agent-id>" }`. When set, the run's stdout is returned to that agent (typically the one that ran `openwizardai-cli send` or `cue trigger --source-agent-id`). **Deprecated** - prefer a downstream `action: command` subscription with `command.mode: cli`.
...
- `mode: cli` is intentionally **local-only**. `openwizardai-cli send` targets the local OpenWizardAI daemon, so SSH-wrapping it would point at the wrong daemon.
...
{{OPENWIZARDAI_CLI_PATH}} cue list [--json]

# Fire a subscription on demand (bypasses its event trigger)
{{OPENWIZARDAI_CLI_PATH}} cue trigger <subscription-name> \
...
4. **Within a pipeline, give each subscription its own `target_node_key`** (any UUID) so the Pipeline Graph renders the chains as separate visual lines instead of collapsing them onto one fan-in agent node. This applies whether the chains share an `agent_id` or not. Only reuse a `target_node_key` across subscriptions when you actually want a real fan-in node (multiple triggers/upstreams converging onto one shared agent node). If two chains genuinely need isolated context/models/project-roots, also give them distinct `agent_id`s (create with `{{OPENWIZARDAI_CLI_PATH}} create-agent <name> --cwd <project>` if needed); otherwise reusing one `agent_id` is fine and often preferred.
5. **For Command nodes (shell scripts or `openwizardai-cli` calls inside a pipeline)** - see the **Command Nodes** section above for the full schema. The keyword is `action: command` plus a `command:` block; there is no separate top-level YAML key, no `event: command` type, and no separate node graph.
6. For full schema, field reference, and worked examples, fetch the official Cue docs: https://github.com/manoelpanev/OpenWizardAI/blob/main/docs/openwizardai-cue-configuration.md, https://github.com/manoelpanev/OpenWizardAI/blob/main/docs/openwizardai-cue-events.md, https://github.com/manoelpanev/OpenWizardAI/blob/main/docs/openwizardai-cue-advanced.md, https://github.com/manoelpanev/OpenWizardAI/tree/main/docs/openwizardai-cue-examples.md. Don't guess field names.
7. After writing, validate with `{{OPENWIZARDAI_CLI_PATH}} cue list` - the engine reloads automatically when the file changes.
8. For anything clock-driven - one-off ("do X at 4pm") or repeating ("every weekday at 9am", "every 30 minutes") - use `{{OPENWIZARDAI_CLI_PATH}} cue schedule` - see the **Scheduled Tasks** section. Never hand-write `time.once`, `time.scheduled`, or `time.heartbeat` subscriptions; let the CLI generate them, and point the user at OpenWizardAI Cue → Scheduled Tasks to manage them.

### Multi-Root Pipelines (agents in different project roots)

A pipeline that spans agents living in **different** project roots is NOT a single-file authoring task. The engine never aggregates yaml files across roots - each agent's runtime only sees `<its-own-projectRoot>/.openwizardai/cue.yaml`. A pipeline split across N agents at N different roots is physically N separate yaml files; the visual Pipeline Graph manages this for you by writing one file per participating agent's cwd on every save.

**When authoring multi-root pipelines by hand, the rule is:**

- **Each subscription lives in its owning agent's local `.openwizardai/cue.yaml`.** "Owning agent" = the agent whose `agent_id` matches the subscription's `agent_id` field. Put the subscription in that agent's project root, not anywhere else.
...
1. List the participating agents and their project roots: `{{OPENWIZARDAI_CLI_PATH}} list agents` (note each agent's `id` and `cwd`).
2. For each agent that owns one or more subscriptions in the pipeline, open / create `<that-agent-cwd>/.openwizardai/cue.yaml`.
...
5. Validate from any agent's workspace with `{{OPENWIZARDAI_CLI_PATH}} cue list` - the command reports every agent's parsed config, so a missing or misplaced sub is immediately visible.
...
When authoring `cue.yaml` for a workspace that may be registered under more than one agent, set `owner_agent_id` proactively. Use `{{OPENWIZARDAI_CLI_PATH}} list agents` to discover ids and names.

### Natural-Language → YAML Recipes

Translate the user's phrasing into one of these starter templates, then adapt names/prompts/agent ids. Always set `agent_id` to the target agent (use `{{OPENWIZARDAI_CLI_PATH}} list agents` to find ids).
...
**"After this agent finishes, send its output to another agent over `openwizardai-cli`" → `action: command` (cli)**
...
After authoring, write the YAML to `<project-root>/.openwizardai/cue.yaml` (create the `.openwizardai/` directory if it doesn't exist), then run `{{OPENWIZARDAI_CLI_PATH}} cue list` to confirm the engine sees it.
