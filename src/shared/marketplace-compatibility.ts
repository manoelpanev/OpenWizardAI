 ...
 * playbook is installable on the running OpenWizardAI version, and whether it
 ...
 *
 * OpenWizardAI's package.json uses uppercase prerelease tags (e.g. "0.16.17-RC")
...
/**
 * Is this playbook installable on the given running OpenWizardAI version?
 *
 * Returns true (compatible) when:
 * - No `minOpenWizardAIVersion` is set, OR
 ...
 * - `runningVersion >= minOpenWizardAIVersion` per semver.gte().
 */
export function isCompatible(playbook: MarketplacePlaybook, runningVersion: string): boolean {
	if (!playbook.minOpenWizardAIVersion) return true;

	const min = normalize(playbook.minOpenWizardAIVersion);
