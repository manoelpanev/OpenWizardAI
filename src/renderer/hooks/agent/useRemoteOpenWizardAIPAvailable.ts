/**
 * useRemoteOpenWizardAIPAvailable.ts
 *
 * Resolve whether `openwizardai-p` is on the PATH of an SSH remote, so the Claude
 ...
 * openwizardai-p`) and caches the result; this hook fetches it on demand.
 *
 *   - `true`      openwizardai-p present on the remote
 ...
 * to the Refresh button so a user who just installed openwizardai-p on the remote can
...
export interface RemoteOpenWizardAIPAvailability {
 ...
 */
export function useRemoteOpenWizardAIPAvailable(
	sshRemoteId: string | null | undefined
): RemoteOpenWizardAIPAvailability {
	...
	useEffect(() => {
		...
		void window.openwizardai.agents
			.getRemoteOpenWizardAIPAvailable(sshRemoteId, force)
			.then((result) => {
				if (!stale) {
			...
			.catch((error: unknown) => {
				logger.error('Failed to probe remote openwizardai-p availability', undefined, error);
				if (!stale) {
					setAvailable(undefined);
				}
			})
