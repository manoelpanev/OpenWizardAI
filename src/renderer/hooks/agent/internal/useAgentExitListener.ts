/**
 * useAgentExitListener - registers `window.openwizardai.process.onExit`
 *
...
export function useAgentExitListener(deps: UseAgentExitListenerDeps): void {
	...
	useEffect(() => {
		...
		const unsubscribe = window.openwizardai.process.onExit(
			async (sessionId: string, code: number) => {
				...
				setSessions((prev) =>
					prev.map((s) => {
						...
						if (isFromAi && orphanIndex !== -1 && s.orphanedThinkingTabs) {
							...
							const orphansWithoutExited = s.orphanedThinkingTabs.filter(
								(_, i) => i !== orphanIndex
							);
							const { item: nextRunnable, remaining: remainingAfterNext } =
								takeNextRunnableQueueItem(s.executionQueue);
				...
				if (queuedItemToProcess) {
					...
					setTimeout(() => {
						window.openwizardai.logger.log('info', 'Agent process completed', 'App', {
