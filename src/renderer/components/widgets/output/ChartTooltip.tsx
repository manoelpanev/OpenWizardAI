 ...
 * (no IPC, no store reads), independent of any Encore flag.
 *
 * Shared portal-based tooltip shell for cursor-anchored chart tooltips. Tooltips
 * rendered inline as `position: fixed; z-50` siblings of the chart work in
 * isolation but suffer when an ancestor briefly holds a non-`none` transform
 * (e.g. during a section-enter animation): the fixed element's containing block
 * becomes that ancestor instead of the viewport, shifting the tooltip far from
 * the cursor and clipping it under later siblings.
 *
