/**
 * OpenWizardAI Prompts Tab - Edit core system prompts
 *
...
import './OpenWizardAIPromptsTab.css';
...
interface OpenWizardAIPromptsTabProps {
	theme: Theme;
...
const CATEGORY_HELP: Record<string, string> = {
	...
	includes:
		'Reusable blocks referenced from other prompts. Two directives consume them: {{INCLUDE:name}} fully inlines the content at assembly time (use for foundational rules every agent must have); {{REF:name}} expands to a one-line pointer that tells the agent to fetch it on demand via `openwizardai-cli prompts get <name>` (use for heavy reference material only some sessions need). Keeps shared content (history format, Auto Run spec, CLI reference, Cue model, file-access rules) in one place so every agent that needs it gets the same wording.',
	system:
		"System-level prompts — the OpenWizardAI system context injected into agents, tab naming, Director's Notes, and feedback.",
};
		...
		<div className="prompts-help-panel" style={{ color: theme.colors.textMain }}>
			...
			<div className="prompts-help-section">
				...
				<p className="prompts-help-text" style={{ color: theme.colors.textDim }}>
					Core prompts are the system instructions that control how OpenWizardAI's AI features
					behave. Each prompt is a Markdown template that gets injected into the AI context for a
					specific feature. Customizing these lets you tailor OpenWizardAI's behavior without
			...
			<div className="prompts-help-section">
				...
				<p className="prompts-help-text" style={{ color: theme.colors.textDim }}>
					...
					<code
						...
						style={{ backgroundColor: theme.colors.bgMain, color: theme.colors.accent }}
					>
						openwizardai-cli prompts get &lt;name&gt;
					</code>{' '}
					instead.
				</p>
			</div>
			...
			>
				...
				<button
					onClick={() =>
						openUrl(
							'https://github.com/manoelpanev/OpenWizardAI/blob/main/docs/prompt-customization.md'
						)
					}
					className="text-xs hover:opacity-80 transition-colors"
					style={{ color: theme.colors.accent }}
				>
					Read more at github.com/manoelpanev/OpenWizardAI/tree/main/docs/prompt-customization
				</button>
			</div>
		</div>
	);
...
export function OpenWizardAIPromptsTab({
	theme,
	initialSelectedPromptId,
	onEscapeHandled,
}: OpenWizardAIPromptsTabProps): JSX.Element {
	...
	useEffect(() => {
		...
		void (async () => {
			try {
				...
				try {
					historyFilePath =
						(await window.openwizardai.history.getFilePath(activeSession.id)) || undefined;
				} catch {
					// ignore
				}
				if (cancelled) return;
				...
				captureException(err instanceof Error ? err : new Error(String(err)), {
					extra: { context: 'OpenWizardAIPromptsTab.buildPreview' },
				});
	...
	const handleToggleShowDefault = useCallback(async () => {
		...
		try {
			const result = await window.openwizardai.prompts.getBundledDefault(selectedPrompt.id);
			...
			captureException(err instanceof Error ? err : new Error(String(err)), {
				extra: { context: 'OpenWizardAIPromptsTab.toggleShowDefault', promptId: selectedPrompt.id },
			});
		...
		(async () => {
			try {
				const [result, pathResult] = await Promise.all([
					window.openwizardai.prompts.getAll(),
					window.openwizardai.prompts.getPath(),
				]);
				...
				if (result.success && result.prompts) {
					...
					const target =
						...
						findById(PROMPT_IDS.OPENWIZARDAI_SYSTEM_PROMPT) ||
						result.prompts[0];
					if (target) {
					...
					captureMessage(`OpenWizardAIPromptsTab load failed: ${msg}`, {
				...
				captureException(err instanceof Error ? err : new Error(String(err)), {
					extra: { context: 'OpenWizardAIPromptsTab.loadPrompts' },
				});
				setError(String(err));
			}
		})();
	...
	const handleSave = useCallback(async () => {
		...
		setError(null);
		try {
			const result = await window.openwizardai.prompts.save(selectedPrompt.id, editedContent);
			if (result.success) {
				...
				captureMessage(`OpenWizardAIPromptsTab save failed: ${msg}`, {
			...
			captureException(err instanceof Error ? err : new Error(String(err)), {
				extra: { context: 'OpenWizardAIPromptsTab.savePrompt', promptId: selectedPrompt.id },
			});
			setError(String(err));
		} finally {
			setIsSaving(false);
	...
	const handleReset = useCallback(async () => {
		...
		setError(null);
		try {
			const result = await window.openwizardai.prompts.reset(selectedPrompt.id);
			if (result.success && result.content) {
				...
				captureMessage(`OpenWizardAIPromptsTab reset failed: ${msg}`, {
					extra: { promptId: selectedPrompt.id, error: result.error },
				});
				setError(msg);
			}
			...
			captureException(err instanceof Error ? err : new Error(String(err)), {
				extra: { context: 'OpenWizardAIPromptsTab.resetPrompt', promptId: selectedPrompt.id },
			});
			setError(String(err));
		} finally {
			...
			{!isEditorExpanded && (
				<div className="prompts-tab-header-text">
					...
					<p className="text-xs opacity-50">
						Customize the system prompts used by OpenWizardAI features. Changes take effect
	...
	return (
		<div className="openwizardai-prompts-settings-tab">
