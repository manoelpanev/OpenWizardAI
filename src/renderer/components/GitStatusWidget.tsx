...
export const GitStatusWidget = memo(function GitStatusWidget({
	...
	// The widget itself, used to position the portaled tooltip beneath it.
	const anchorRef = useRef<HTMLDivElement>(null);

	/** Cancel a pending close (pointer re-entered the widget or the tooltip). */
	const keepOpen = () => {
	...
	/** Close after a grace period so the pointer can travel to the tooltip. */
	const scheduleClose = () => {
	...
	return (
		...
		>
			...
			{/* Changed-files tooltip. Portaled out of the header: the Main Panel
			    header clips its left cluster with overflow-hidden, which used to
			    swallow this entire panel. See useAnchoredMenuPosition. */}
...
/**
 * The changed-files panel, portaled to document.body.
 *
 * Split into its own component so the positioning hook runs on mount (hooks
 * can't be called conditionally). The old inline version also carried an
 * invisible "bridge" div to cover the gap between widget and panel; that's
 * unnecessary here because the 150ms close delay already covers the few pixels,
 * and the bridge itself was clipped and therefore never worked.
 */
function GitStatusTooltip({
	...
	const { left, top, ready } = useAnchoredMenuPosition(tooltipRef, anchorRef);

	return createPortal(
		<div
			ref={tooltipRef}
			className="fixed w-max max-w-[400px] rounded shadow-xl z-[100] pointer-events-auto"
			style={{
				left,
				top,
				opacity: ready ? 1 : 0,
