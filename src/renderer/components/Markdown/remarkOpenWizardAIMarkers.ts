/**
 * Turn OpenWizardAI's Auto Run markers into renderable nodes so a document preview
...
export const MARKER_DATA_ATTRIBUTES = {
	kind: 'dataOpenwizardaiMarker',
	status: 'dataOpenwizardaiMarkerStatus',
	scope: 'dataOpenwizardaiMarkerScope',
	label: 'dataOpenwizardaiMarkerLabel',
	detail: 'dataOpenwizardaiMarkerDetail',
	artifact: 'dataOpenwizardaiMarkerArtifact',
	reason: 'dataOpenwizardaiMarkerReason',
} as const;
...
function indexMarkers(source: string): Map<string, ScannedMarker> {
	...
	for (const marker of scanOpenWizardAIMarkers(source)) {
...
function markerKindOf(value: string): ScannedMarker['kind'] | undefined {
	if (/<!--\s*OPENWIZARDAI:HITL\b/i.test(value)) return 'hitl';
	if (/<!--\s*openwizardai:halt\b/i.test(value)) return 'halt';
	if (/<!--\s*OPENWIZARDAI:MODEL\b/i.test(value)) return 'model';
	return undefined;
}

export function remarkOpenWizardAIMarkers() {
	return (tree: Root, file: VFile) => {
		const source = String(file?.value ?? '');
		if (!source || !hasOpenWizardAIMarker(source)) return;
