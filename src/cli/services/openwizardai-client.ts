// CLI WebSocket client for communicating with the running OpenWizardAI desktop app.
...
export class UnsupportedCommandError extends Error {
	...
	constructor(commandType: string) {
		super(
			`The running OpenWizardAI app does not support the '${commandType}' command. ` +
...
export class CommandTimeoutError extends Error {
	...
	constructor(responseType: string) {
		super(
			`Timed out waiting for the OpenWizardAI app to respond (expected '${responseType}'). ` +
...
export class OpenWizardAIClient {
	private ws: WebSocket | null = null;
	private pendingRequests: Map<string, PendingRequest> = new Map();

	/**
	 * Connect to the running OpenWizardAI app.
	 * Throws if the app is not running or connection fails.
	 */
	async connect(): Promise<void> {
		const info = readCliServerInfo();
		if (!info) {
			throw new Error('OpenWizardAI desktop app is not running');
		}

		if (!isCliServerRunning()) {
			throw new Error('OpenWizardAI discovery file is stale (app may have crashed)');
		}
		...
		return new Promise<void>((resolve, reject) => {
			...
			const timeout = setTimeout(() => {
				if (!settled) {
					settled = true;
					ws.close();
					reject(new Error('Connection to OpenWizardAI timed out'));
				}
			}, CONNECT_TIMEOUT_MS);
			...
			ws.on('error', (err) => {
				...
				settled = true;
				clearTimeout(timeout);
				reject(new Error(`Failed to connect to OpenWizardAI: ${err.message}`));
			});
		});
	}
	...
	async sendCommand<T>(
		...
		if (!this.ws || this.ws.readyState !== WebSocket.OPEN) {
			throw new Error('Not connected to OpenWizardAI');
		}
...
export function resolveSessionId(options: { session?: string }): string {
	...
	if (sessions.length === 0) {
		console.error('Error: No agents found. Create an agent in OpenWizardAI first.');
		process.exit(1);
	}
 ...
 */
export async function withOpenWizardAIClient<T>(
	action: (client: OpenWizardAIClient) => Promise<T>
): Promise<T> {
