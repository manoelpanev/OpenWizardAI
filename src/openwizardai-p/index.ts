...
// interactive TUI under the hood, so callers (OpenWizardAI, shells, pipelines)
...
// See OPENWIZARDAI-P-01-binary.md for the full contract.
...
program
	...
	.description(
		[
			...
			'  - openwizardai-p flags (consumed):    --status, --stream-thinking, --max-wait, --first-byte-timeout, --help, --version',
			...
			'',
			'Environment:',
			'  OPENWIZARDAI_CLAUDE_BIN  Path to the claude binary (defaults to `claude` on PATH).',
			'  CLAUDE_CONFIG_DIR   Claude config directory (defaults to ~/.claude); inherited by the TUI.',
		].join('\n')
	)
	.version(VERSION, '-v, --version', 'Print the openwizardai-p version and exit')
...
/**
 * True when `p` points at openwizardai-p itself (by basename), so we never try to
 * drive openwizardai-p as if it were the claude TUI.
 */
function isOpenWizardAIPSelfPath(p: string): boolean {
	const base = p.replace(/\\/g, '/').split('/').pop() || p;
	return base === 'openwizardai-p' || base === 'openwizardai-p.js' || base === 'openwizardai-p.exe';
}

function resolveBinPath(): string {
	const envBin = process.env.OPENWIZARDAI_CLAUDE_BIN;
	// Self-reference guard: when an agent's configured binary IS openwizardai-p (the
	// supported "openwizardai-p Path" way to force the TUI), the desktop can pass that
	// same path through OPENWIZARDAI_CLAUDE_BIN. Honoring it would make openwizardai-p spawn
	...
	// openwizardai-p). Fall back to `claude` on PATH in that case.
	if (envBin && envBin.length > 0 && !isOpenWizardAIPSelfPath(envBin)) {
		return envBin;
	}
	return 'claude';
}
...
async function runMode(args: ParsedArgs): Promise<never> {
	if (!args.prompt || args.prompt.length === 0) {
		process.stderr.write(
			'openwizardai-p: no prompt provided. Use a positional argument, -p/--prompt, or pipe a prompt on stdin.\n'
		);
		process.exit(1);
	}

	// `--input-format stream-json` mode: parse the Claude envelope OpenWizardAI
	...
	if (args.streamJsonInput) {
		...
		if (translated) {
			...
			process.stderr.write(
				'openwizardai-p: --input-format stream-json was set but stdin was not a valid Claude stream-json envelope; treating it as a plain-text prompt.\n'
			);
		}
	}
	...
	driver.on('api-billing', () => {
		...
		process.stderr.write(
			`openwizardai-p: warning: ${diagnosis.reason} This turn is billed as per-token API credit, not plan quota.\n`
		);
	});
	...
	const finalize = (options: { isError: boolean; error?: string; exitCode: number }): void => {
		...
		try {
			...
			process.stderr.write(
				`openwizardai-p: failed to emit result envelope: ${(err as Error).message}\n`
			);
		}
	...
	const processEntry = (entry: unknown): void => {
		...
		if (e.type === 'assistant' && message) {
			...
			// dialog openwizardai-p can't answer, so `end_turn` never arrives and the
		...
		if (e.type === 'user' && message) {
			...
			if (!promptEchoChecked) {
				...
				if (echo !== null) {
					...
					if (mismatch) {
						process.stderr.write(
							`openwizardai-p: claude received a damaged prompt (${mismatch.receivedBytes} of ${mismatch.sentBytes} bytes; first missing text: "${mismatch.missingFrom}"). The terminal dropped part of the input. Stopping the turn and failing with prompt_truncated.\n`
						);
						driver.kill('SIGTERM');
	...
	const handleParseError = (payload: ParseErrorPayload): void => {
		...
		process.stderr.write(
			`openwizardai-p: JSONL parse error: ${payload.error.message} - line: ${snippet}\n`
		);
	};
...
async function statusMode(args: ParsedArgs): Promise<never> {
	...
	const driver = new TuiDriver({
		...
		captureScreen: true,
		// Set only by OpenWizardAI's usage sampler, which runs this from a dedicated
		...
		acceptWorkspaceTrust: process.env.OPENWIZARDAI_P_ACCEPT_WORKSPACE_TRUST === '1',
	});
	...
	driver.on('exit', () => {
		...
		process.stderr.write('openwizardai-p: claude TUI exited before /usage panel could render\n');
		process.exit(1);
	});
	...
	// Diagnostic hook: when OPENWIZARDAI_P_DUMP_RAW points at a file, write the raw
	...
	const dumpRaw = (raw: string, attempt: number): void => {
		const dumpPath = process.env.OPENWIZARDAI_P_DUMP_RAW;
		if (!dumpPath) return;
		try {
	...
	if (apiBilling) {
		...
		process.stderr.write(`openwizardai-p: ${reason} There is no plan usage to report.\n`);
	} else {
		...
		process.stderr.write(
			`openwizardai-p: failed to parse /usage output after ${attempts} ` +
...
// promotes it to an uncaught exception - and because openwizardai-p runs under
...
main().catch((err) => {
	...
	process.stderr.write(`openwizardai-p: ${message}\n`);
	process.exit(1);
});
