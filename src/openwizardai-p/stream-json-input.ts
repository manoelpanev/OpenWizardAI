...
//
// OpenWizardAI pipes that envelope to us whenever the user attaches an image to a
// Claude Code agent backed by `openwizardai-p` (see
...
//
// Output: `@/tmp/openwizardai-p-image-…-0.png @/tmp/openwizardai-p-image-…-1.jpeg\n<text>`
...
function extToFilename(mediaType: string, index: number): string | null {
	...
	return `openwizardai-p-image-${process.pid}-${Date.now()}-${index}.${ext}`;
}

/**
 * Parse a stream-json envelope produced by OpenWizardAI's
		...
		if (type === 'image') {
			...
			try {
				...
				process.stderr.write(
					`openwizardai-p: failed to write stream-json image to ${fullPath}: ${(err as Error).message}\n`
				);
			}
		}
	}
