// Argument resolution for openwizardai-p.
//
// Walks argv once and partitions tokens into three buckets:
//   (a) consumed   - openwizardai-p's own flags (-p/--print/--prompt, --status,
...
//
// Flag-guard for value-taking openwizardai-p flags
// -------------------------------------------
// `consumeValue()` (used by `-p`/`--print`/`--prompt`, `--max-wait`, and
// `--input-format`) refuses to swallow a next-slot token that itself starts
// with `-`. This is what protects the opt-in openwizardai-p route: when a user
// points their Claude Code custom path at `openwizardai-p`, OpenWizardAI spawns it with
...
export interface ParsedArgs {
	...
	/**
	 * True when invoked with `--input-format stream-json`. OpenWizardAI sets this
...
function defaultReadStdin(): string {
	...
	// the bare readFileSync crashed openwizardai-p with "EAGAIN ... read" - surfaced to
...
export function parseArgs(argv: string[], options: ParseArgsOptions = {}): ParsedArgs {
	...
	while (i < argv.length) {
		...
		// like a flag (`-x` / `--foo`). When OpenWizardAI forwards its API-mode
		// claude args verbatim to a custom-path openwizardai-p (the opt-in route),
		...
		// positional, never a flag. OpenWizardAI's API-mode spawn line ends with
		...
		if (PROMPT_VALUE_FLAGS.has(flag)) {
			...
			if (value === undefined) {
				warn(`openwizardai-p: ${flag} requires a value; ignoring.`);
			} else {
		...
		if (flag === '--max-wait') {
			...
			if (value === undefined) {
				warn(
					`openwizardai-p: --max-wait requires a value; using default ${DEFAULT_MAX_WAIT_SECONDS}s.`
				);
			} else {
				...
				if (Number.isFinite(parsed) && parsed > 0) {
					...
					warn(
						`openwizardai-p: --max-wait "${value}" is not a positive integer; using default ${DEFAULT_MAX_WAIT_SECONDS}s.`
					);
				}
			}
			i += 1;
		...
		if (flag === '--first-byte-timeout') {
			...
			if (value === undefined) {
				warn(
					`openwizardai-p: --first-byte-timeout requires a value; using default ${DEFAULT_FIRST_BYTE_TIMEOUT_SECONDS}s.`
				);
			} else {
				const parsed = Number.parseInt(value, 10);
				if (Number.isFinite(parsed) && parsed > 0) {
					...
					warn(
						`openwizardai-p: --first-byte-timeout "${value}" is not a positive integer; using default ${DEFAULT_FIRST_BYTE_TIMEOUT_SECONDS}s.`
					);
				}
			}
			i += 1;
		...
		if (STRIPPED_VALUE_FLAGS.has(flag)) {
			warn(`openwizardai-p: ignoring ${flag} - headless-mode flag, not forwarded to the TUI.`);
		...
		if (flag === '--input-format') {
			...
			if (value === 'stream-json') {
				...
				warn(`openwizardai-p: --input-format requires a value; ignoring.`);
			} else {
				warn(
					`openwizardai-p: ignoring --input-format ${value} - only stream-json is recognized; stdin will be treated as plain text.`
				);
			}
			i += 1;
			continue;
