// Environment for the claude TUI openwizardai-p spawns.
//
// openwizardai-p inherits whatever environment its caller built, and headless callers
...
// session. When openwizardai-p is invoked from within a Claude agent (or any
...
// Since the JSONL is openwizardai-p's only source of truth, the run produces no
...
// and config (CLAUDE_CONFIG_DIR, ANTHROPIC_*, OPENWIZARDAI_CLAUDE_BIN) are kept.
