// Session-id discovery for openwizardai-p run mode (fresh sessions).
//
...
export interface DiscoverSessionIdOptions {
	 ...
	 * earliest-new-file heuristic can attach one openwizardai-p instance to a
 ...
 * openwizardai-p bundle is intentionally lean and the shared module pulls in
 * Electron-adjacent helpers we don't need. The duplication is kept honest by
 * a parity test in src/__tests__/openwizardai-p/session-watcher.test.ts that asserts
