...
/// Adapter für Claude Code. Referenzimplementierung für die übrigen
/// Adapter (siehe ARCHITECTURE.md, Abschnitt "Tool-Adapter-Design").
...
impl ToolAdapter for ClaudeCodeAdapter {
    ...
    fn generate_files(&self, config: &WizardConfig) -> Vec<FileWrite> {
        let content = format!(
            "# {}\n\nProjekt-Kontext für Claude Code. Von OpenWizardAI generiert.\n",
            config.project_name
        );
    ...
    fn project_hitl_level(&self, level: HitlLevel) -> Vec<FileWrite> {
        ...
        // Claude Code hat keinen eigenen strukturierten Permission-Modus in
        // CLAUDE.md — die Stufe wird als Hinweistext projiziert, der beim
